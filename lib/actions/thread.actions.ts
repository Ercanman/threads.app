...
export async function createThread({text, author, community_Id, path}: Params) {
    try {
        connectToDB();
...
export async function fetchPosts(pageNumber = 1, pageSize = 20) {
    ...
    }).sort({createdAt: "desc"}).skip(skipAmount).limit(pageSize).populate({path: "author", model: User}).populate({path: "children", populate: {
        path: "author",
        model: User,
